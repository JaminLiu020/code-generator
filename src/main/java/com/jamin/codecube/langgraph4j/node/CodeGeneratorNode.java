    ...
    public static AsyncNodeAction<MessagesState<String>> create() {
        return node_async(state -> {
            ...
            if (generationType.equals(CodeGenTypeEnum.HTML) || generationType.equals(CodeGenTypeEnum.MULTI_FILE)) {
                codeStream.blockLast(Duration.ofMinutes(15)); // 最多等待 15 分钟
            } else if(generationType.equals(CodeGenTypeEnum.VUE_PROJECT)) {
                codeStream.blockLast(Duration.ofMinutes(30)); // 最多等待 30 分钟
            }
            else {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "不支持的代码生成类型: " + generationType);
            }
            // 根据类型设置生成目录
