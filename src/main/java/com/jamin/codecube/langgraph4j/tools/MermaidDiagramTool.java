...
public class MermaidDiagramTool {
    ...
    private File convertMermaidToSvg(String mermaidCode) {
        // 构建临时目录
        File mermaidDir = FileUtil.mkdir(System.getProperty("user.dir") + File.separator + "tmp" + File.separator + "mermaid");
        // 创建临时输入文件
        File tempInputFile = FileUtil.createTempFile("mermaid_input_", ".mmd", mermaidDir, true);
        FileUtil.writeUtf8String(mermaidCode, tempInputFile);
        // 创建临时输出文件
        File tempOutputFile = FileUtil.createTempFile("mermaid_output_", ".svg", mermaidDir, true);
        // 根据操作系统选择命令
